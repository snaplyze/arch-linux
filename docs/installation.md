...
`install.sh` from the immutable `1.0.2` tag, never from `main`, and the bootstrap then downloads and
...
```bash
curl -fsS https://raw.githubusercontent.com/snaplyze/arch-linux/1.0.3/install.sh | bash -s -- --verify-only
```
