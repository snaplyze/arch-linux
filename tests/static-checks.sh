...
with tempfile.TemporaryDirectory(prefix='public-source-') as raw:
    ...
    (root / 'repository' / 'README.md').write_text('release documentation\n')
    (root / 'repository' / 'verify.sh').write_text('original verifier\n')
    git('add', '.')
    ...
    (root / 'repository' / 'README.md').write_text('reviewed release readback documentation\n')
    git('add', '.')
    git('commit', '-m', 'release documentation')
    assert run().stdout == release + ':' + git('rev-parse', 'HEAD')
    (root / 'repository' / 'verify.sh').write_text('changed verifier\n')
    git('add', '.')
    git('commit', '-m', 'verifier change')
    assert run().returncode != 0
    (root / 'repository' / 'verify.sh').write_text('original verifier\n')
    git('add', '.')
    git('commit', '-m', 'restore exact verifier')
    assert run().stdout == release + ':' + git('rev-parse', 'HEAD')
