...
def write_once(path, data):
    demand(not path.exists() and not path.is_symlink(), f"refusing to replace output: {path}")
...
def file_binding(path, expected=None):
    meta = safe_file(path)
    value = (meta.st_dev, meta.st_ino), sha_file(path)
    demand(expected is None or value == expected, "staged verdict identity/hash changed")
    return value
...
def finalize_command(args, current_source=None):
    ...
    write_once(pending, encoded)
    binding = file_binding(pending)
    sync_dir(root)
    remove_tree(raw)
    file_binding(pending, binding)
    os.rename(pending, output)
    sync_dir(root)
    file_binding(output, binding)
    demand(tree_size(common) == value["cumulativePermanentEvidenceBytes"], "cumulative permanent evidence size changed")
...
def _selftest_run():
    ...
    stage = run / "staged-verdict"
    create(stage, b"verdict")
    stage_binding = file_binding(stage)
    alias = run / "staged-verdict-alias"
    os.link(stage, alias)
    reject(file_binding, stage, stage_binding)
    alias.unlink()
    replace(stage, b"verdict")
    reject(file_binding, stage, stage_binding)
    stage.unlink()
