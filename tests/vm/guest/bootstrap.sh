...
write_config() {
    ...
    [[ "${IDENTITY[SCENARIO]}" != *-grub ]] || bootloader='grub'
    {
        ...
        printf 'ARCH_LINUX_ROOT_PARTITION=%s\n' "$(partition_name "${target}" 2)"
        printf 'ARCH_LINUX_DISK_IDENTITY=%s\n' "${target_identity}"
        printf 'ARCH_LINUX_BOOT_PARTITION_IDENTITY=\n'
        printf 'ARCH_LINUX_ROOT_PARTITION_IDENTITY=\n'
        printf 'ARCH_LINUX_FILESYSTEM=%s\n' "${filesystem}"
        printf 'ARCH_LINUX_BOOTLOADER=%s\n' "${bootloader}"
        printf 'ARCH_LINUX_DUAL_BOOT_ENABLED=false\n'
...
main() {
    ...
    *) fail 'scenario identity is invalid' ;;
    ...
    if [ "${IDENTITY[INPUT_MODE]}" = staged ] && \
        [ "${IDENTITY[SCENARIO]}" = marble-gnome-btrfs-luks2-plymouth-systemdboot ]; then
